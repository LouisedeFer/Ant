...
def test_board_creation()-> None :
    ...
    dic_init={(0,0) : ant.Tile(" ",WHITE), (1,0) : ant.Tile("X",BLACK)}
...
def test_board_change_color() -> None : 
    ...
    assert ant.board.Board(dic_init).dictionary[(0,0)].color==BLACK


def test_board_add_tile() -> None :
    """Test if the tiles are added properly."""
    dic_init={(0,0) : ant.Tile(" ",WHITE), (1,0) : ant.Tile("X",BLACK)}
    dic_1={(0,0) : ant.Tile(" ",WHITE), (1,0) : ant.Tile("X",BLACK), (0,1) : ant.Tile(" ", WHITE)}
